import AllAdscategory from "@/components/AllAdscategory";
import AllRelatedcategory from "@/components/AllRelatedcategory";
import Ads from "@/components/ads";
import { fetchAd, fetchAdsByCategory } from "@/lib/fetchData";
import Link from "next/link";
//import Link from "next/link";

export default async function page({ params }: { params: { adID: string } }) {
  const adID = params.adID;
  const ad: any = await fetchAd(adID);
  const Adscatg = await fetchAdsByCategory(ad.category);

  return (
    <div className="max-w-5xl mx-auto">
      <div className="text-sm breadcrumbs">
        <ul>
          <li>
            <a href="/" className="hover:text-green-700 no-underline">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                className="w-4 h-4 mr-2 stroke-current"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                ></path>
              </svg>
              Home
            </a>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              className="w-4 h-4 mr-2 stroke-current"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
              ></path>
            </svg>
            <Link
              className="hover:text-green-700 no-underline"
              href={`/category/${ad.category}`}
            >
              {ad.category}
            </Link>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              className="w-4 h-4 mr-2 stroke-current"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"
              ></path>
            </svg>
            {ad.title}
          </li>
        </ul>
      </div>

      <Ads {...ad} />
      <div className="text-lg font-bold breadcrumbs">Related Ads</div>

      <AllRelatedcategory ads={Adscatg} />
    </div>
  );
}
